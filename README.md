...
- **100%可靠性**：成功处理143个文件，全部成功（使用Gemini 2.5 Flash + gcli2api实现）
- **性能提升79.6%**：相比OpenRouter版本（25文件5失败）显著提升可靠性
- **多API支持**：Google Gemini、OpenRouter、阿里Qwen，支持多种LLM提供商
- **智能文本分割**：自动按章节标题分割长文本文件，支持多种书籍格式
- **并行处理加速**：多线程并发处理，大幅提升转换效率
- **情感向量生成**：自动生成8维情感向量，优化TTS语音合成质量
- **Speaker分类**：AI自动分类说话人（少男、少女、中男、中女、老男、老女），便于配音

这是一个完整的电子书转有声书工作流工具集，实现TXT2CHAP（文本分割章节）、TXT2JSON（转换为JSON格式）、JSONPersonClassify（说话人分类）三大核心功能，将普通电子书转换为可用于index-tts v2的有声书JSON格式。
